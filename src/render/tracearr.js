...
class _TraceaRrMethods {
  ...
  _traMapHtml() {
    ...
    const streams  = Number(summary.totalStreams)    || 0;
    const locs     = Number(summary.uniqueLocations) || 0;
  ...
  async _traLoadTab(tab, modal) {
    ...
    if (tab === 'map') {
      ...
      const _sQ = m.mapServerId ? `&serverId=${encodeURIComponent(m.mapServerId)}` : '';
      const _uQ = m.mapUserId  ? `&userId=${encodeURIComponent(m.mapUserId)}`    : '';
